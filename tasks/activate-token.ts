...
task('lz:oapp:activate-token', 'Activates a token on the destination chain')
    .addParam('tokenid', 'Token ID to activate', undefined, types.string)
    ...
    .setAction(async (args: { tokenid: string; options?: string }, hre: HardhatRuntimeEnvironment) => {
        ...
        logger.info(`Token ID to activate: ${args.tokenid}`)
        ...
        let activationManagerContract
        let contractAddress: string
        try {
            const activationManagerDeployment = await hre.deployments.get('ActivationManager')
            contractAddress = activationManagerDeployment.address
            activationManagerContract = await hre.ethers.getContractAt('ActivationManager', contractAddress, signer)
            logger.info(`ActivationManager contract found at: ${contractAddress}`)
        } catch (error) {
            DebugLogger.printErrorAndFixSuggestion(
                KnownErrors.ERROR_GETTING_DEPLOYMENT,
                `Failed to get ActivationManager deployment on network: ${hre.network.name}`
            )
            throw error
        }
        ...
        logger.info('Approving token for activation...')

        const tokenDeployment = await hre.deployments.get('MockNFT')
        const tokenContract = await hre.ethers.getContractAt('MockNFT', tokenDeployment.address)

        await tokenContract.approve(contractAddress, args.tokenid)

        logger.info('Token activation transaction...')
        let tx: ContractTransaction
        try {
            tx = await activationManagerContract.activateToken(args.tokenid, 200_000)
            logger.info(`  Transaction hash: ${tx.hash}`)
        } catch (error) {
            DebugLogger.printErrorAndFixSuggestion(
                KnownErrors.ERROR_SENDING_TRANSACTION,
                `For token ID: ${args.tokenid}, Contract: ${contractAddress}`
            )
            throw error
        }
